// ============================================================
// FILE: trainer/src/backend/cuda_ops.cu
// ============================================================
...
__global__ void k_silu_bf16(const __nv_bfloat16* __restrict__ x,
...
void silu_bf16(const __nv_bfloat16* x, __nv_bfloat16* out, int n, cudaStream_t s) {
    k_silu_bf16<<<(n+BLOCK-1)/BLOCK, BLOCK, 0, s>>>(x, out, n);
}

__global__ void k_silu_mul_bf16(const __nv_bfloat16* __restrict__ gate,
...
__global__ void k_silu_mul_bwd(const float* __restrict__ d_out,
    ...
    float silu_g = sig * g;
    // d_gate = d_out * up * silu'(g) where silu'(g) = sig*(1 + g*(1-sig))
    d_gate[i] = do_ * u * sig * (1.0f + g * (1.0f - sig));
    d_up[i]   = do_ * silu_g;
}

void silu_mul_backward_fp32(const float* d_out, const float* gate, const float* up,
                             float* d_gate, float* d_up, int n, cudaStream_t s) {
    k_silu_mul_bwd<<<(n+BLOCK-1)/BLOCK, BLOCK, 0, s>>>(d_out, gate, up, d_gate, d_up, n);
}

// RMSNorm forward: one warp per row, iterative reduction
__global__ void k_rmsnorm_fwd(const __nv_bfloat16* __restrict__ x,
...
void rmsnorm_bf16(const __nv_bfloat16* x, const __nv_bfloat16* w, __nv_bfloat16* out,
                  float* rms, int rows, int dim, float eps, cudaStream_t s) {
    k_rmsnorm_fwd<<<rows, BLOCK, 0, s>>>(x, w, out, rms, rows, dim, eps);
}

// RMSNorm backward
__global__ void k_rmsnorm_bwd(const float* __restrict__ d_out,
    ...
    // dot(d_out_scaled, x_scaled) for the sum term
    float dot = 0.0f;
    for (int i = threadIdx.x; i < dim; i += blockDim.x) {
...
void rmsnorm_backward_fp32(const float* d_out, const __nv_bfloat16* x,
                            ...
                            float* d_x, int rows, int dim, cudaStream_t s) {
    k_rmsnorm_bwd<<<rows, BLOCK, 0, s>>>(d_out, x, w, rms, d_x, rows, dim);
}

// Softmax (fp32, row-wise)
__global__ void k_softmax(float* __restrict__ x, int rows, int cols) {
    ...
    for (int i = threadIdx.x; i < cols; i += blockDim.x) {
        float v = expf(xr[i] - maxv); xr[i] = v; sumv += v;
    }
...
__global__ void k_softmax_bwd(const float* __restrict__ p,
...
void softmax_backward_fp32(const float* p, const float* d_out,
                            float* d_x, int rows, int cols, cudaStream_t s) {
    k_softmax_bwd<<<rows, BLOCK, 0, s>>>(p, d_out, d_x, rows, cols);
}

// Cast kernels
__global__ void k_bf16_to_f32(const __nv_bfloat16* src, float* dst, int n) {
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i < n) dst[i] = __bfloat162float(src[i]);
}
__global__ void k_f32_to_bf16(const float* src, __nv_bfloat16* dst, int n) {
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i < n) dst[i] = __float2bfloat16(src[i]);
}

void cast_bf16_to_fp32(const __nv_bfloat16* src, float* dst, int n, cudaStream_t s) {
    k_bf16_to_f32<<<(n+BLOCK-1)/BLOCK, BLOCK, 0, s>>>(src, dst, n);
}
void cast_fp32_to_bf16(const float* src, __nv_bfloat16* dst, int n, cudaStream_t s) {
    k_f32_to_bf16<<<(n+BLOCK-1)/BLOCK, BLOCK, 0, s>>>(src, dst, n);
}

__global__ void k_add_fp32(const float* a, const float* b, float* c, int n) {
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i < n) c[i] = a[i] + b[i];
}
void add_fp32(const float* a, const float* b, float* c, int n, cudaStream_t s) {
    k_add_fp32<<<(n+BLOCK-1)/BLOCK, BLOCK, 0, s>>>(a, b, c, n);
}

__global__ void k_scale_fp32(float* x, float s, int n) {
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i < n) x[i] *= s;
}
void scale_fp32(float* x, float s, int n, cudaStream_t s_) {
    k_scale_fp32<<<(n+BLOCK-1)/BLOCK, BLOCK, 0, s_>>>(x, s, n);
}

// Simple transpose using shared memory tile
#define TILE 32
__global__ void k_transpose_bf16(const __nv_bfloat16* __restrict__ src,
...
void transpose_bf16(const __nv_bfloat16* src, __nv_bfloat16* dst,
    ...
    k_transpose_bf16<<<grid, block, 0, s>>>(src, dst, M, N);
}
